...
class OvisController : public rclcpp::Node {
public:
    OvisController() : Node("ovis_controller_node") {
        // Buttons
        ...
        joy_sub_ = create_subscription<sensor_msgs::msg::Joy>(
            "/joy", 10, std::bind(&OvisController::joyCallback, this, std::placeholders::_1)
        );

        joy_pub_ = create_publisher<sensor_msgs::msg::Joy>("/ovis/joy", 10);
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<OvisController>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
